...
from webnet.plugins.registry import PluginRegistry
...
class TestPluginBase:
    ...
    def test_plugin_metadata_validation(self):
        ...
        assert plugin.verbose_name == "Test Plugin"
    ...
    def test_plugin_default_methods(self, test_plugin):
        """Test default method implementations."""
        assert test_plugin.get_models() == []
        assert test_plugin.get_api_viewsets() == []
        assert test_plugin.get_ui_views() == []
        assert test_plugin.get_navigation_items() == []
        assert test_plugin.get_dashboard_widgets() == []
        assert test_plugin.get_settings_schema() == {}
    
    def test_plugin_lifecycle_hooks(self, test_plugin):
        """Test plugin lifecycle hooks are called."""
        assert not test_plugin.load_called
        test_plugin.on_load()
        assert test_plugin.load_called
        
        assert not test_plugin.enable_called
        test_plugin.on_enable()
        assert test_plugin.enable_called
        
        assert not test_plugin.disable_called
        test_plugin.on_disable()
        assert test_plugin.disable_called
        
        assert not test_plugin.unload_called
        test_plugin.on_unload()
        assert test_plugin.unload_called
    
    def test_plugin_health_check(self, test_plugin):
...
class TestPluginRegistry:
    ...
    def test_register_plugin(self, plugin_registry, test_plugin):
        ...
        assert test_plugin.name in plugin_registry.get_all_plugins()
    
    def test_get_plugin(self, plugin_registry, test_plugin):
        """Test getting a plugin by name."""
        plugin_registry.register_plugin(test_plugin)
        retrieved = plugin_registry.get_plugin(test_plugin.name)
    ...
    def test_load_plugin(self, plugin_registry, test_plugin):
        """Test loading a plugin."""
        plugin_registry.register_plugin(test_plugin)
        success = plugin_registry.load_plugin(test_plugin.name)
        assert success is True
        assert test_plugin.load_called is True
        assert plugin_registry.is_plugin_loaded(test_plugin.name)
    
    def test_unload_plugin(self, plugin_registry, test_plugin):
        """Test unloading a plugin."""
        plugin_registry.register_plugin(test_plugin)
        plugin_registry.load_plugin(test_plugin.name)
        success = plugin_registry.unload_plugin(test_plugin.name)
        assert success is True
        assert test_plugin.unload_called is True
        assert not plugin_registry.is_plugin_loaded(test_plugin.name)
    
    def test_load_nonexistent_plugin(self, plugin_registry):
        """Test loading a plugin that doesn't exist."""
        success = plugin_registry.load_plugin("nonexistent")
        assert success is False
    
    def test_get_all_plugins(self, plugin_registry, test_plugin):
        """Test getting all plugins."""
        plugin_registry.register_plugin(test_plugin)
        plugins = plugin_registry.get_all_plugins()
        assert test_plugin.name in plugins
        assert plugins[test_plugin.name] is test_plugin
...
class TestPluginManager:
    ...
    def test_sync_plugins(self, plugin_registry, test_plugin):
        """Test syncing plugins to database."""
        plugin_registry.register_plugin(test_plugin)
        PluginManager.sync_plugins()
        
        config = PluginConfig.objects.get(name=test_plugin.name)
        assert config.verbose_name == test_plugin.verbose_name
        assert config.version == test_plugin.version
    
    def test_enable_plugin_globally(self, plugin_registry, test_plugin, admin_user):
        ...
        assert success is True
        assert test_plugin.enable_called is True
        
        config = PluginConfig.objects.get(name=test_plugin.name)
        assert config.enabled is True
        
        # Check audit log
        log = PluginAuditLog.objects.filter(
            plugin=config,
            action="enable"
        ).first()
    ...
    def test_disable_plugin_globally(self, plugin_registry, test_plugin, admin_user):
        ...
        assert success is True
        assert test_plugin.disable_called is True
        
        config = PluginConfig.objects.get(name=test_plugin.name)
        assert config.enabled is False
    
    def test_enable_plugin_for_customer(self, plugin_registry, test_plugin, customer, admin_user):
        ...
        customer_config = CustomerPluginConfig.objects.get(
            plugin__name=test_plugin.name,
            customer=customer
        )
        assert customer_config.enabled is True
    
    def test_is_plugin_enabled(self, plugin_registry, test_plugin, customer):
        ...
        config = PluginConfig.objects.get(name=test_plugin.name)
        assert PluginManager.is_plugin_enabled(test_plugin.name) is True
        
        # Disable globally
        config.enabled = False
        config.save()
        assert PluginManager.is_plugin_enabled(test_plugin.name) is False
    
    def test_update_plugin_settings(self, plugin_registry, test_plugin, admin_user):
    ...
    def test_get_plugin_health(self, plugin_registry, test_plugin):
        """Test getting plugin health status."""
        plugin_registry.register_plugin(test_plugin)
        plugin_registry.load_plugin(test_plugin.name)
        
        health = PluginManager.get_plugin_health(test_plugin.name)
        assert health["healthy"] is True
        assert health["message"] == "OK"
...
class TestPluginAPI:
    ...
    def test_list_plugins(self, client, admin_user, plugin_registry, test_plugin):
    ...
    def test_enable_plugin_via_api(self, client, admin_user, plugin_registry, test_plugin):
    ...
    def test_disable_plugin_via_api(self, client, admin_user, plugin_registry, test_plugin):
    ...
    def test_plugin_health_via_api(self, client, admin_user, plugin_registry, test_plugin):
