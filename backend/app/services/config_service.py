...
class ConfigService:
    ...
    def get_snapshot_multi_tenant(
        self,
        snapshot_id: int,
        context: MultiTenantContext,
    ) -> ConfigSnapshot:
        """Get a configuration snapshot by ID, supporting multi-tenant access."""
        customer_ids = context.customer_ids
        if not customer_ids:
            raise NotFoundError("Snapshot not found")

        snapshot = self.snapshots.get_by_id_for_customers(snapshot_id, customer_ids)
        if not snapshot:
            raise NotFoundError("Snapshot not found")
    ...
    def list_device_snapshots_multi_tenant(
        self,
        device_id: int,
        context: MultiTenantContext,
        limit: int = 100,
    ) -> Sequence[ConfigSnapshot]:
        """List configuration snapshots for a device, supporting multi-tenant access."""
        customer_ids = context.customer_ids
        if not customer_ids:
            raise NotFoundError("Device not found")

        device = self.snapshots.get_device_with_customers_check(device_id, customer_ids)
        if not device:
            raise NotFoundError("Device not found")
    ...
    def get_config_diff_multi_tenant(
        self,
        device_id: int,
        from_snapshot_id: int,
        to_snapshot_id: int,
        ...
        """Get diff between two configuration snapshots, supporting multi-tenant access."""
        customer_ids = context.customer_ids
        if not customer_ids:
            raise NotFoundError("Device not found")
        ...
        # Get both snapshots - use the customers check for security
        snapshot_from = self.snapshots.get_by_id_for_customers(from_snapshot_id, customer_ids)
        snapshot_to = self.snapshots.get_by_id_for_customers(to_snapshot_id, customer_ids)
